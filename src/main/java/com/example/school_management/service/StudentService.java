...
/*public ResponseDto getStudentByPages(final int pageNumber, final int pageSize, final boolean order, final String name) {
        Pageable pageable = PageRequest.of(pageNumber, pageSize, Sort.by(order ? Sort.Direction.ASC : Sort.Direction.DESC, name));
    ...
    public List<Student> getStudentByName(String search){
        return this.studentRepository.findStudentByNameOrCity(search);
        }
   /*
//    public ResponseDto getStudentBySortingAndSearching(final int pageNumber, final int pageSize, final boolean order, final String sortField, final String search) {
//        Pageable pageable = PageRequest.of(pageNumber, pageSize, Sort.by(order ? Sort.Direction.ASC : Sort.Direction.DESC, sortField));
//        Page<Student> studentPage = this.studentRepository.findStudentByNameOrCity(search, pageable);//
//        return ResponseDto.builder()
//                .message(Constants.RETRIEVED)
//                .data(studentPage) // Only return one object with pagination
//                .statusCode(HttpStatus.OK.value())
//                .build();
//    }

/*public Page<Student> getStudentPage(final int pageIndex, final int pageSize){
